...
const MasterEquipmentTypeModel = {
    // เมธอดสำหรับดึงข้อมูลผู้ใช้ทั้งหมด
    getAllMasterEquipmentType: async () => {
        try {
            const result = await db.query('SELECT * FROM master_equipments_type');
            return result.rows;
        } catch (err) {
            throw new Error('Error fetching all master Equipments Type: ' + err.message);
        }
    },
...
module.exports = MasterEquipmentTypeModel;